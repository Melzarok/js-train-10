...
function customObjectFromEntries(entries) {
  ...
  const test = entries.map(([key, value]) => {
  ...
  const resultObj = Object.fromEntries(test);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  return Object.isFrozen(newObj) ? newObj : `об'єкт не було "заморожено"`;
}
...
function filterObjectsByKey(arr, key) {
  ...
  const filterKeys = arr.filter((elem) => elem.hasOwnProperty(key));
...
function getObjectValuesSum(obj) {
  ...
  const sumValues = allValuesObj.reduce((acc, elem) => acc + elem);
